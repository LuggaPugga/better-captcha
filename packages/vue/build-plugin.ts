...
function generateComponentFiles(metadata: ProviderMetadata): GeneratedFiles {
	...
	const initializer = `createCaptchaComponent(${providerClassName})`;
...
function generateComponentDts(metadata: ProviderMetadata): string {
	const { name, componentName, handleType, renderParamsType, renderParamsOmit, extraTypes, identifierProp = "sitekey" } = metadata;
	...
	const propsTypeModifier = identifierProp === "endpoint" 
		? `Omit<CaptchaProps<Omit<${renderParamsType}, ${renderParamsOmit}>>, "sitekey"> & { endpoint: string }`
		: `CaptchaProps<Omit<${renderParamsType}, ${renderParamsOmit}>>`;
...
export declare const ${componentName}: DefineComponent<${propsTypeModifier}, CaptchaEmits<${handleType}>>;
