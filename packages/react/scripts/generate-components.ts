...
import { PROVIDER_REGISTRY, type ProviderMetadata } from "@react-captcha/core/src/registry";
...
function generateComponentFile(metadata: ProviderMetadata): string {
	...
	return `// Auto-generated from @react-captcha/core
...
} from "@react-captcha/core/providers/${name}";
...
function generateAggregateIndexFile(providers: ProviderMetadata[]): string {
	const lines = ["// Auto-generated from @react-captcha/core", "// Do not edit this file directly", ""];
...
function logProviderInfo(metadata: ProviderMetadata): void {
	console.log(`✅ Generated: ${metadata.componentName} (${metadata.name})`);
...
function main(): void {
	console.log("🚀 Generating React components from core providers...\n");
	...
	for (const metadata of providers) {
		try {
			...
			console.error(`❌ Failed to generate: ${metadata.componentName} - ${error}`);
			process.exit(1);
		}
	}
	...
	try {
		...
		console.log(`✨ Successfully generated ${providers.length} React component${providers.length !== 1 ? "s" : ""}!`);
	} catch (error) {
		console.error(`❌ Failed to write aggregate provider index: ${error}`);
		process.exit(1);
	}
}
