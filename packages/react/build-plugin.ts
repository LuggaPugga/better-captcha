...
function generateReactAggregateIndexDts(providers: ProviderMetadata[]): string {
	return generateAggregateIndexFile(providers, ".js");
}

function generateReactAggregateIndexJs(providers: ProviderMetadata[]): string {
		...
		rollup: {
			generateBundle() {
				...
				this.emitFile({
					...
					source: generateReactAggregateIndexJs(PROVIDER_REGISTRY),
				});

				this.emitFile({
					type: "asset",
					fileName: "provider/index.d.ts",
					source: generateReactAggregateIndexDts(PROVIDER_REGISTRY),
				});
			},
		},
	};
