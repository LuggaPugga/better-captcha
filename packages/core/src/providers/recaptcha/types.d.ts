export interface RenderParameters {
	...
	/**
	 * Enterprise checkbox reCAPTCHA only.
	 * The action associated with the user-initiated event.
	 */
	action?: string | undefined;
	/**
	 * Invisible reCAPTCHA only. For plugin owners to not interfere with existing reCAPTCHA installations on a page.
	...
	/**
	 * Your callback function that's executed when the reCAPTCHA response expires and the user needs to solve a new CAPTCHA.
	 */
	// Notice to the reader
	// I need to surround this object with quotes, this will however break intellisense in VS 2013.
	"expired-callback"?(): void;
	/**
	 * Your callback function that's executed when reCAPTCHA encounters an error (usually network connectivity) and cannot continue until connectivity is restored.
...
export declare namespace ReCaptcha {
	interface ReCaptcha {
		 ...
		 */
		render(container: string | HTMLElement, parameters?: RenderParameters, inherit?: boolean): number;
		/**
	...
	// auto is not natively supported by the reCAPTCHA API, but it is supported by the better-captcha library.
