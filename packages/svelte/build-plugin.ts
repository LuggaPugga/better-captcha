...
function generateSvelteComponent(metadata: ProviderMetadata): string {
	const { name, providerClassName, handleType, renderParamsType, renderParamsOmit, useEndpoint } = metadata;
...
function generateComponentDts(metadata: ProviderMetadata): string {
	...
	const propType = useEndpoint ? "endpoint: string;" : "sitekey: string;";
	const exampleProp = useEndpoint ? 'endpoint="https://example.com/api/cap"' : 'sitekey="your-site-key"';
