		...
		declarations: [
			{
				...
				initializer: `$((sitekey: string) => new ${meta.providerClassName}(sitekey))`,
			},
		],
	});
...
function genProviderModuleCjs(meta: ProviderMetadata): string {
	...
	lines.push(`    (sitekey) => new ${meta.providerClassName}(sitekey),`);
	lines.push(");");
	lines.push("");
	lines.push(`exports.${meta.componentName} = createCaptchaComponent(${meta.componentName}Factory);`);
...
function genIndexDts(): string {
	...
	// Add CaptchaProps type with JSDoc
	...
	// Export createCaptchaComponent
	sourceFile.addExportDeclaration({
		namedExports: ["createCaptchaComponent"],
		moduleSpecifier: "./base-captcha.qwik.mjs",
	});
...
function genBaseCaptchaDts(): string {
	...
	// Add CaptchaProps type
	sourceFile.addTypeAlias({
		name: "CaptchaProps",
		typeParameters: [{ name: "TOptions" }, { name: "THandle", constraint: "CaptchaHandle" }],
		type: `{
	sitekey: string;
	options?: TOptions;
	class?: string;
	...
	// Add createCaptchaComponent function declaration
	sourceFile.addFunction({
		...
		parameters: [
			{
				...
				type: "QRL<(sitekey: string) => TProvider>",
			},
		],
