...
const qwikConfig: FrameworkConfig = {
	...
	componentCreation: (providerClassName: string, useEndpoint?: boolean) => {
		return useEndpoint
			? `$((endpoint: string) => new ${providerClassName}(endpoint))`
			: `$((sitekey: string) => new ${providerClassName}(sitekey))`;
	},
...
function genProviderModule(meta: ProviderMetadata): string {
	...
	sourceFile.addImportDeclaration({
		namedImports: [meta.useEndpoint ? "createCaptchaComponentWithEndpoint" : "createCaptchaComponent"],
		...
		declarations: [
			{
				...
				initializer: `${meta.useEndpoint ? "createCaptchaComponentWithEndpoint" : "createCaptchaComponent"}(${meta.componentName}Factory)`,
			},
		],
	});
...
function genProviderModuleCjs(meta: ProviderMetadata): string {
	...
	lines.push(`const { ${meta.useEndpoint ? "createCaptchaComponentWithEndpoint" : "createCaptchaComponent"} } = require("../../base-captcha.qwik.cjs");`);
	...
	lines.push("");
	lines.push(
		`exports.${meta.componentName} = ${meta.useEndpoint ? "createCaptchaComponentWithEndpoint" : "createCaptchaComponent"}(${meta.componentName}Factory);`,
	);
