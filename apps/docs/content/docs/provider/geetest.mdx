---
title: Geetest
description: Geetest provides a robust CAPTCHA solution to protect your website from bots and automated abuse.
---

import { GeetestPlayground } from "../../../src/components/geetest-playground";

## About Geetest

GeeTest adaptive CAPTCHA offers an intelligent solution to distinguish bots from humans, preventing fraud and threats. Powered by machine learning and AI, it provides a seamless and secure experience in every interaction.

For more information, visit the [official Geetest documentation](https://www.geetest.com/en/adaptive-captcha).

## Showcase
<GeetestPlayground />

## Usage

<Tabs items={['React', 'Preact', 'Qwik', 'SolidJS', 'Vue', 'Svelte', 'Lit']} groupId="framework">
  <Tab value="React">
  ```tsx
  import { Geetest } from "@better-captcha/react/provider/geetest";

  <Geetest sitekey="08649cc61c7078689263ebf78225d616" options={{ language: "eng" }} />
  ```
  </Tab>

  <Tab value="Preact">
  ```tsx
  import { Geetest } from "@better-captcha/preact/provider/geetest";

  <Geetest sitekey="08649cc61c7078689263ebf78225d616" options={{ language: "eng" }} />
  ```
  </Tab>
  
  <Tab value="Qwik">
  ```tsx
  import { component$ } from "@builder.io/qwik";
  import { Geetest } from "@better-captcha/qwik/provider/geetest";

  export default component$(() => (
    <Geetest sitekey="08649cc61c7078689263ebf78225d616" options={{ language: "eng" }} />
  ));
  ```
  </Tab>
  
  <Tab value="SolidJS">
  ```tsx
  import { Geetest } from "@better-captcha/solidjs/provider/geetest";

  <Geetest sitekey="08649cc61c7078689263ebf78225d616" options={{ language: "eng" }} />
  ```
  </Tab>

  <Tab value="Vue">
  ```vue
  <template>
    <Geetest
      sitekey="08649cc61c7078689263ebf78225d616"
      :options="{ language: 'eng' }"
    />
  </template>

  <script setup lang="ts">
  import { Geetest } from "@better-captcha/vue/provider/geetest";
  </script>
  ```
  </Tab>

  <Tab value="Svelte">
  ```svelte
  <script lang="ts">
  import { Geetest } from "@better-captcha/svelte/provider/geetest";
  </script>

  <Geetest sitekey="08649cc61c7078689263ebf78225d616" options={{ language: "eng" }} />
  ```
  </Tab>

  <Tab value="Lit">
  ```ts
  import { Geetest } from "@better-captcha/lit/provider/geetest";
  ```

  ```html
  <geetest-captcha sitekey="08649cc61c7078689263ebf78225d616" .options={{ language: "eng" }}></geetest-captcha>
  ```
  </Tab>
</Tabs>

## Configuration

### Provider Options

<AutoTypeTable path="../../packages/core/src/providers/geetest/types.d.ts" name="RenderParameters" />

### Imperative Handle

<AutoTypeTable path="../../packages/core/src/providers/geetest/provider.ts" name="GeetestHandle" />
