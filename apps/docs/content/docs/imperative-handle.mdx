...
<Tabs items={['React', 'Qwik', 'SolidJS', 'Vue']} groupId="framework">
  ...
  export default component$(() => {
    const handle = useSignal<ReCaptchaHandle | null>(null);

    return (
      <>
        <ReCaptcha 
          sitekey="6LeIxAcTAAAAAJcZVRqyHh71UMIEGNQ_MXjiZKhI" 
          onReady$={(h) => (handle.value = h)}
        />
        <button onClick$={() => handle.value?.execute()}>Execute</button>
        <button onClick$={() => handle.value?.reset()}>Reset</button>
        <button onClick$={() => console.log(handle.value?.getResponse())}>Log response</button>
      </>
    );
  });
  ```
